...
static void inline new_sha256(uint8_t * p_data_out, const uint8_t * p_data_in, const int data_in_len)
{
	uint8_t value_bin[128] = {0};
	int i;
	...
	if (data_in_len == 65)
	{
		memcpy(value_bin, p_data_in, 65);
		value_bin[65] = 0x80;
		value_bin[126] = 0x02;
		value_bin[127] = 0x08;
	}
	else if (data_in_len == 33)
	{
		memcpy(value_bin, p_data_in, 33);
		value_bin[33] = 0x80;
		value_bin[62] = 0x01;
		value_bin[63] = 0x08;
	}
	...
	{
		printf("Invalid sha256 data length input");
		exit(EXIT_FAILURE);
	}
	__builtin_memcpy(p_data_out, __sha256_init, 32);
	__sha256_osol(value_bin, (uint32_t*)p_data_out);
	if (data_in_len > 48) __sha256_osol(&value_bin[64], (uint32_t*)p_data_out);	
	for (i = 0; i < 8; ++i) {
		((uint32_t*)p_data_out)[i] = __bswap_32(((uint32_t*)p_data_out)[i]);
	}
